package com.example.vijaya.earthquakeapp;

import  android.util.Log;
import org.json.JSONArray;
import org.json.JSONObject;
import java.io.BufferedReader;
import java.io.InputStream;
import java.io.InputStreamReader;
import java.net.HttpURLConnection;
import java.net.URL;
import java.nio.charset.Charset;
import java.util.ArrayList;
import java.util.List;

public class QueryUtils {
    /**
     * Tag for the log messages
     */
    private static final String LOG_TAG = QueryUtils.class.getSimpleName();

    /**
     * Create a private constructor because no one should ever create a {@link QueryUtils} object.
     * This class is only meant to hold static variables and methods, which can be accessed
     * directly from the class name QueryUtils (and an object instance of QueryUtils is not needed).
     */
    private QueryUtils() {
    }

    /**
     * Query the USGS dataset and return a list of {@link Earthquake} objects.
     */
    public static List<Earthquake> fetchEarthquakeData2(String requestUrl) {
        // An empty ArrayList that we can start adding earthquakes to
        List<Earthquake> earthquakes = new ArrayList<>();
        try {
            //TODO: 1. Create a URL from the requestUrl string and make a GET request to it
            //  URL object to store the url for a given string
            URL requestURL = new URL(requestUrl);

            HttpURLConnection URLConnection = (HttpURLConnection) requestURL.openConnection();
            URLConnection.setReadTimeout(10000);
            URLConnection.setConnectTimeout(15000);
            URLConnection.setRequestMethod("GET");
            URLConnection.connect();

            //TODO: 2. Read from the Url Connection and store it as a string
            int URLResponse = URLConnection.getResponseCode();
            InputStream inputStream = null;

            if (URLResponse == HttpURLConnection.HTTP_OK){
                inputStream = URLConnection.getInputStream();
            }
            // store the response obtained from rest call in the form of string
            StringBuilder responseString = new StringBuilder();
            InputStreamReader inputStreamReader = new InputStreamReader(inputStream, Charset.forName("UTF-8"));
            BufferedReader bufferedReader = new BufferedReader(inputStreamReader);
            String line = null;

            while ((line = bufferedReader.readLine()) != null) {
                responseString.append(line);
            }

                /*TODO: 3. Parse the jsonResponse string obtained in step 2 above into JSONObject to extract the values of
                        "mag","place","time","url"for every earth quake and create corresponding Earthquake objects with them
                        Add each earthquake object to the list(earthquakes) and return it.
                */

            JSONObject jsonObject = new JSONObject(responseString.toString());
            JSONArray jsonArray = jsonObject.getJSONArray("features");

            for (int eQuake = 0; eQuake < jsonArray.length(); eQuake++){
                JSONObject tempJson = jsonArray.getJSONObject(eQuake);
                JSONObject properties = tempJson.getJSONObject("properties");

                Double storedMag = properties.getDouble("mag");
                String storedPlace = properties.getString("place");
                Long storedTime = properties.getLong("time");
                String storedURL = properties.getString("url");

                Earthquake tempEarthQuake = new Earthquake(storedMag,storedPlace, storedTime, storedURL);
                earthquakes.add(tempEarthQuake);
            }
        } catch (Exception e) {
            Log.e(LOG_TAG, "Exception:  ", e);
        }
        // Return the list of earthquakes
        return earthquakes;
    }
}
